...
export class AdminReportesComponent implements OnInit {
  ...
  constructor(private reportes: AdminReportesService, private router: Router) {}
  ...
  estadoBadge(e: string): 'ok' | 'warn' | 'danger' {
    ...
    if (v.includes('confirm') || v.includes('respond') || v.includes('cerrad')) return 'ok';
    if (v.includes('pend') || v.includes('proceso') || v.includes('nueva')) return 'warn';
  ...
  verOperacion(tipo: 'reservas' | 'solicitudes', estado: string) {
    const returnTo = this.router.url;
