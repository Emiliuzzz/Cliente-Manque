...
export class MisSolicitudesService {
  ...
  getMisReservas(): Observable<MisReserva[]> {
    return this.http.get<any>(`${this.apiRoot}/mis-reservas/`).pipe(
      map((resp) => {
  ...
  getSolicitudesCliente(): Observable<SolicitudClienteResumen[]> {
    return this.http
      .get<any>(`${this.apiRoot}/mis-solicitudes-cliente/`)
      .pipe(
        map((resp) => {
