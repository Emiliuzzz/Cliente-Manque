...
export class MisPropiedadesService {
  ...
  getMisPropiedades(): Observable<MisPropiedad[]> {
    return this.http.get<any>(`${this.baseUrl}/mis-propiedades/`).pipe(
      map((resp) => {
  ...
  getMiPerfilPropietario() {
    return this.http.get<PerfilPropietario>(`${this.apiRoot}/mi-perfil-propietario/`);
  }

  actualizarMiPerfilPropietario(payload: Partial<PerfilPropietario>) {
    return this.http.put<PerfilPropietario>(`${this.apiRoot}/mi-perfil-propietario/`, payload);
  }
