...
export class AdminContratoDetalleComponent implements OnInit {
  ...
  private cargarCombos() {
    this.http.get<any>(`${this.baseUrl}/propiedades/?disponibles_contrato=true`).subscribe({
      next: (r) => (this.propiedades = this.toArray(r)),
      error: () => (this.propiedades = []),
    });

    this.http.get<any>(`${this.baseUrl}/interesados/`).subscribe({
  ...
  private parseError(e: any): string | null {
    ...
    volverAtras(): void {
      // Si viene returnTo (queryParam), volvemos ahí; si no, volvemos a la lista
      if (this.returnTo) {
